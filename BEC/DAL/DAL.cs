...
namespace DALayer
{
      ...
      public class DALItem
      {
          ...
          public bool ItemInsert(BELItem beobj)
          {
              ...
              cmd.CommandText = "Insert into tbl_ItemMast values(@name,@code,@mrp,@exp,0)";
              cmd.Parameters.Clear();
              cmd.Parameters.AddWithValue("@name", beobj._Itemname);
          ...
          public bool ItemUpdate(BELItem beobj)
          {
              ...
              cmd.CommandText = "Update Tbl_ItemMast set Item_name=@name,Item_code=@code,Mrp=@mrp,exp=@exp where Item_id=@Itemid";
              cmd.Parameters.Clear();
              ...
              try
              {
                  ...
                  for (int i = 0; i <= beobj._lstItems.Count - 1; i++)
                  {
                      ...
                      for (int j = 0; j <= Convert.ToDouble(str[6]) - 1; j++)
                      {
                          ...
                          cmd.CommandText = "Insert into tbl_barcode values(@serialno,@recdate,@comp,@recno,@itemcode,@batch,@exp,@mrp,@prate,'',0,'')";
                          cmd.Parameters.Clear();
                       ...
                       for (int i = 0; i <= beobj._lstItems.Count - 1; i++)
                       {
                           ...
                           if (dt.Rows.Count > 0)
                           {
                               for (int j = 0; j <= dt.Rows.Count - 1; j++)
                               {
                                   ...
                                   cmd.CommandText = "Insert into tbl_desp_barcode values(@desno,@itemcode,@serialno,@batch,@expiry,0)";
                                   cmd.Parameters.Clear();
      ...
      public class DALRecRep 
      {
          ...
          public SqlDataReader RecReport(BELRecRep beobj)
          {
              ...
              cmd.CommandType = CommandType.Text;
              cmd.CommandText = "EXEC dbo.RecieptRep @Item=@item,@from=@fromdate,@to=@todate";
      ...
      public class DALDespRep
      {
          ...
          public SqlDataReader despReport(BELDespRep beobj)
          {
              ...
              cmd.CommandType = CommandType.Text;
              cmd.CommandText = "EXEC dbo.DespRep @Item=@item,@from=@fromdate,@to=@todate";
              cmd.Parameters.AddWithValue("@item", beobj._name);
              cmd.Parameters.AddWithValue("@fromdate", beobj._from.ToString("yyyy-MM-dd"));
              cmd.Parameters.AddWithValue("@todate", beobj._to.ToString("yyyy-MM-dd"));
              SqlDataReader da;
