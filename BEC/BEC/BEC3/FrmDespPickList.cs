        ...
        private void cmdSave_Click(object sender, EventArgs e)
        {
            if (cmdSupp.SelectedValue != null && cmbComp.Text != null)
            {
                if (c1TrueDBGrid1.Rows.Count != 0)
                {
                    ...
                    { COMP = "BIO MEDICAL ENGINEERING COMPANY"; }
                    //DataTable dt = objbll.ReturningDataByQ("select NoofPacks from dbo.tbl_barcode where Serial_no =");
                    for (int i = 0; i <= c1TrueDBGrid1.Rows.Count - 1; i++)
                    {
                    ...
                    else
                    {
                            ...
                            if (dialogResult == DialogResult.Yes)
                        {
                            ...
                            if (da.Rows.Count != 0)
                            {
                                ...
                                rpt.SetDataSource(da); rpt.SetParameterValue("@Comp", COMP);
